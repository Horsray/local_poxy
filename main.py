...
atexit.register(cleanup_silent)
...
if platform.system() == "Windows":
    try:
        ...
        def silent_exit_handler(ctrl_type):
            if ctrl_type in (
        ...
        win32api.SetConsoleCtrlHandler(silent_exit_handler, True)

    except ImportError:
        pass 
from main_payload import init_payload
init_payload()
import subprocess
def hide_temp_dir():
    try:
...
class HuiYingProxy:
    def __init__(self, config_file='config.json', mappings_file=None):
        global temp_dir
        if mappings_file is None:
            mappings_file = os.path.join(temp_dir, "workflow_mappings.json")
        self.config_file = config_file
        self.mappings_file = mappings_file
        self.config = self.load_config(config_file)
        self.mappings = self.load_mappings(self.mappings_file)
        self.workflow_cache = {}
...
def log_all_requests():
    logger.info(f"📡 收到绘影接口请求: {request.method} {request.path}")
...
if __name__ == '__main__':
    ...
    logger.info("✅ HTTP & WebSocket 服务启动成功：http://0.0.0.0:8080")
