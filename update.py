...
def fetch_remote_version():
    print(f"🔗 获取远程版本信息: {VERSION_URL}")
    try:
        r = requests.get(VERSION_URL, timeout=10)
        if r.status_code == 200:
            return r.text.strip()
        print(f"❌ 获取远程版本失败 ({VERSION_URL}): 状态码 {r.status_code}")
    except Exception as e:
        print(f"❌ 获取远程版本异常 ({VERSION_URL}): {e}")
    return None
...
def _extract_version_from_payload(data: bytes) -> str | None:
    """Return version contained in payload archive if available."""
    try:
        decrypted = decrypt(base64.b64decode(data), KEY)
        with zipfile.ZipFile(io.BytesIO(decrypted)) as zf:
            for name in zf.namelist():
...
def download_payload(version: str | None = None) -> bool:
    """Download payload.b64 and update local version file."""
    print(f"🔗 下载更新包: {PAYLOAD_URL}")
    try:
        ...
        if version and not verify_payload(r.content, version):
            print('❌ 更新包校验失败')
            return False

        if not version:
            version = _extract_version_from_payload(r.content)
